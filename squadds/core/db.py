...
if sys.platform == "darwin":  # Checks if the operating system is macOS
    warnings.filterwarnings(
        "ignore", category=UserWarning, module="pyaedt"
    )  # ANSYS is not a mac product
...
class SQuADDS_DB(metaclass=SingletonMeta):
    ...
    def upload_dataset(self, file_paths, repo_file_names, overwrite=False):
        ...
        for file_path, repo_file_name in zip(file_paths, repo_file_names):
            if repo_file_name in existing_files and not overwrite:
                print(
                    f"File {repo_file_name} already exists in the repository. Skipping upload."
                )
                continue
            try:
                api.upload_file(
    ...
    def supported_config_names(self):
        ...
        configs = get_dataset_config_names(
            self.repo_name, download_mode="force_redownload"
        )
    ...
    def view_datasets(self):
        ...
        component_urls = [
            f"https://github.com/LFL-Lab/SQuADDS/tree/master/docs/_static/images/{name}.png"
    ...
    def view_measured_devices(self):
        ...
        rows = [
            [device_info[header] for header in headers]
        ...
        print(
            tabulate(
                rows, headers=headers, tablefmt="grid", stralign="left", numalign="left"
            )
        )
    ...
    def view_contributors_of_config(self, config):
        ...
        for contrib_info in configs_contrib_info:
            # Extracting the relevant information
            relevant_info = {
                key: contrib_info[key]
                for key in ["uploader", "PI", "group", "institution"]
            }
    ...
    def get_device_contributors_of(
        self, component=None, component_name=None, data_type=None
    ):
        """
    ...
    def view_device_contributors_of(
        self, component=None, component_name=None, data_type=None
    ):
        """
        View the reference/source experimental device that was used to validate a specific simulation configuration.
        ...
        for contrib_info, foundry, recipe, github_url in zip(
            ...
            if contrib_info["name"] == device_name:
                ...
                data = [
                    ...
                    ["Fabrication Recipe Links", github_url],
                ]
    ...
    def view_reference_devices(self):
        ...
        for contrib_info in configs_contrib_info:
            ...
            relevant_info = {
                key: contrib_info[key] for key in ["name", "group", "measured_by"]
            }
    ...
    def select_system(self, components=None):
        ...
        if isinstance(components, list):
            for component in components:
                if component not in self.supported_components():
                    print(
                        f"Component `{component}` not supported. Available components are:"
                    )
                    print(self.supported_components())
                    return
                else:
            ...
            if components not in self.supported_components():
                print(
                    f"Component `{components}` not supported. Available components are:"
                )
                print(self.supported_components())
                return
            else:
    ...
    def select_cavity_claw(self, cavity=None):
        ...
        if (self.selected_system == "cavity_claw") or (
            "cavity_claw" in self.selected_system
        ):
        ...
        if self.selected_cavity not in self.supported_component_names():
            print(
                f"Cavity `{self.selected_cavity}` not supported. Available cavities are:"
            )
    ...
    def select_coupler(self, coupler=None):
        ...
        if self.selected_coupler not in self.supported_component_names() + [
            "CLT"
        ]:  # TODO: handle dynamically

            print(
                f"Coupler `{self.selected_coupler}` not supported. Available couplers are:"
            )
    ...
    def see_dataset(self, data_type=None, component=None, component_name=None):
        ...
        component_name = (
            ...
            else self.selected_component_name
        )
    ...
    def get_dataset(
        self, data_type=None, component=None, component_name=None, async_mode=False
    ):
        if async_mode:
            return asyncio.run(
                self._get_dataset_async(data_type, component, component_name)
            )
        else:
            return self._get_dataset_sync(data_type, component, component_name)

    def create_system_df(self, parallelize=False, num_cpu=None, async_mode=False):
        if async_mode:
            return asyncio.run(self._create_system_df_async(parallelize, num_cpu))
        else:
            return self._create_system_df_sync(parallelize, num_cpu)

    def generate_qubit_half_wave_cavity_df(
        self, parallelize=False, num_cpu=None, save_data=False, async_mode=False
    ):
        if async_mode:
            return asyncio.run(
                self._generate_qubit_half_wave_cavity_df_async(
                    parallelize, num_cpu, save_data
                )
            )
        else:
            return self._generate_qubit_half_wave_cavity_df_sync(
                parallelize, num_cpu, save_data
            )

    def _get_dataset_sync(self, data_type=None, component=None, component_name=None):
        # Use the instance attributes if the user does not provide them
        component = component if component is not None else self.selected_system
        component_name = (
            component_name
    ...
    async def _get_dataset_async(
        self, data_type=None, component=None, component_name=None
    ):
        # Use the instance attributes if the user does not provide them
        component = component if component is not None else self.selected_system
        component_name = (
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.repo_name}/{config}") as response:
    ...
    def _create_system_df_sync(self, parallelize=False, num_cpu=None):
        """
    ...
    async def _create_system_df_async(self, parallelize=False, num_cpu=None):
        """
        Creates and returns a DataFrame based on the selected system.
        ...
        if isinstance(self.selected_system, str):
            df = await self._create_single_component_df()
        elif isinstance(self.selected_system, list):
            df = await self._create_multi_component_df(parallelize, num_cpu)
        else:
            raise UserWarning(
                "Selected system is either not specified or does not contain a cavity! Please check `self.selected_system`"
            )
    ...
    async def generate_updated_half_wave_cavity_df(
        self, parallelize=False, num_cpu=None
    ):
        """
        !TODO: speed this up!
        """
        cavity_df = await self.get_dataset(
            data_type="eigenmode",
            component="cavity_claw",
            component_name=self.selected_cavity,
        )

        assert self.selected_coupler in [
            "NCap",
            "CapNInterdigitalTee",
        ...
        if not all(cavity_df["coupler_type"] == "NCap"):
            raise ValueError(
                "All entries in the 'coupler_type' column of the cavity_df must be 'NCap'."
            )
    ...
    async def _generate_qubit_half_wave_cavity_df_async(
        self, parallelize=False, num_cpu=None, save_data=False
    ):
        """
        ...
        tasks = [
            self.get_dataset(
                ...
                component="qubit",
                component_name=self.selected_qubit,
            ),
            self.generate_updated_half_wave_cavity_df(
                parallelize=parallelize, num_cpu=num_cpu
            ),
        ]
        qubit_df, cavity_df = await asyncio.gather(*tasks)
        ...
        df = self.create_qubit_cavity_df(
            qubit_df,
            cavity_df,
            merger_terms=self.claw_merger_terms,
        ...
        print(
            f"Memory usage reduced by {100*(initial_mem - final_mem)/initial_mem:.2f}%"
        )

        if save_data:
            await self._save_data(opt_df, cavity_df, df)

        return opt_df

    def _generate_qubit_half_wave_cavity_df_sync(
        self, parallelize=False, num_cpu=None, save_data=False
    ):
        """
        Generates a DataFrame that combines the qubit and half-wave cavity data.
        ...
        """
        print("Generating half-wave-cavity DataFrame...")
        qubit_df = self.get_dataset(
            data_type="cap_matrix",
            component="qubit",
            component_name=self.selected_qubit,
        )
        cavity_df = self.generate_updated_half_wave_cavity_df(
            parallelize=parallelize, num_cpu=num_cpu
        )
        ...
        if save_data:
            self._save_data(opt_df, cavity_df, df)

        return opt_df

    async def _save_data(self, opt_df, cavity_df, df):
        async with aiofiles.open("data/half-wave-cavity_df.parquet", "wb") as file:
            await file.write(cavity_df.to_parquet())
        async with aiofiles.open(
            "data/qubit_half-wave-cavity_df_uncompressed.parquet", "wb"
        ) as file:
            await file.write(df.to_parquet())
        async with aiofiles.open(
            "data/qubit_half-wave-cavity_df.parquet", "wb"
        ) as file:
            await file.write(opt_df.to_parquet())

    async def _create_multi_component_df(self, parallelize, num_cpu):
        ...
        if self.selected_coupler == "CLT":
            cavity_df = filter_df_by_conditions(
                cavity_df, {"coupler_type": self.selected_coupler}
            )
            self.cavity_df = cavity_df
        if self.selected_coupler == "NCap":
            self.coupler_df = await self.get_dataset(
                data_type="cap_matrix",
                component="coupler",
                component_name=self.selected_coupler,
            )
    ...
    def _update_cap_interdigital_tee_parameters(self, cavity_df):
        ...
        ncap_df = self.get_dataset(
            data_type="cap_matrix", component="coupler", component_name="NCap"
        )
        ncap_sim_cols = [
            "bottom_to_bottom",
            "bottom_to_ground",
        ...
        df = update_ncap_parameters(
            cavity_df, ncap_df, self.ncap_merger_terms, ncap_sim_cols
        )
        return df

    def create_qubit_cavity_df(
        self, qubit_df, cavity_df, merger_terms=None, parallelize=False, num_cpu=None
    ):
        """
                ...
                merged_df_parts = list(
                    tqdm(
                        pool.starmap(
                            merge_dfs,
                            [
                                (split, cavity_df, merger_terms)
                                for split in qubit_df_splits
                            ],
                        ),
                        total=n_cores,
                    )
        ...
        merged_df["design_options"] = merged_df.apply(
            create_unified_design_options, axis=1
        )
    ...
    def _set_target_param_keys(self, df):
        ...
        if self.selected_system is None:
            raise UserWarning(
                "No selected system df is created. Please check `self.selected_df`"
            )
        else:
            ...
            # check if target_param_keys is type list and system has more than one element
                ...
                isinstance(self.target_param_keys, list)
                and len(self.selected_system) == 2
            ):
                self.target_param_keys += get_sim_results_keys(df)
            # check if target_param_keys is type list and system has only one element
            elif (
                isinstance(self.target_param_keys, list)
                and len(self.selected_system) != 1
            ):
                self.target_param_keys = get_sim_results_keys(df)
            else:
                raise UserWarning(
                    "target_param_keys is not None or a list. Please check `self.target_param_keys`"
                )

            # update the attribute to remove any elements that start with "unit"
            self.target_param_keys = [
                key for key in self.target_param_keys if not key.startswith("unit")
            ]
    ...
    def read_parquet_file(self, file_name):
        ...
        """
        base_url = (
            f"https://huggingface.co/datasets/{self.repo_name}/resolve/main/{file_name}"
        )
