...
def get_design_from_ml_predictions(analyzer, test_data, y_pred_dnn):
    ...
    for i, row in test_data.iterrows():
        ...
        qubit_design_options = (
            closest_qubit_claw_design["design_options_qubit"].iloc[0].copy()
        )
        cavity_design_options = (
            closest_cavity_cpw_design["design_options_cavity_claw"].iloc[0].copy()
        )
        ...
        qubit_design_options["connection_pads"]["readout"]["claw_length"] = (
            f"{claw_length_pred}um"
        )
        qubit_design_options["connection_pads"]["readout"]["ground_spacing"] = (
            f"{ground_spacing_pred}um"
        )
        ...
        device_dict["design_options"]["qubit_options"]["connection_pads"]["readout"][
            "claw_cpw_length"
        ] = "0um"
...
def process_dataframe(analyzer, target_params_df, index, path_to_file):
    ...
    new_df = merged_df.drop(
        columns=[col for col in merged_df.columns if col not in columns_to_keep]
    )
    ...
    new_df["ground_spacing"] = merged_df["design_options"].apply(
        lambda x: float(
            x["qubit_options"]["connection_pads"]["readout"]["ground_spacing"][:-2]
        )
    )
    new_df["coupling_length"] = merged_df["design_options"].apply(
        lambda x: float(
            x["cavity_claw_options"]["coupler_options"]["coupling_length"][:-2]
        )
    )
    new_df["total_length"] = merged_df["design_options"].apply(
        lambda x: float(
            x["cavity_claw_options"]["cpw_opts"]["left_options"]["total_length"][:-2]
        )
    )
    ...
    if analyzer.selected_resonator_type == "quarter":
        ...
        for index in range(len(target_params_df)):
            processed_dfs.append(
                process_dataframe(analyzer, target_params_df, index, path_to_file)
            )
        ...
        if path_to_file:
            if path_to_file.endswith(".parquet"):
                ...
                raise ValueError(
                    "Please provide a valid file format (.parquet or .csv) for the training data."
                )
            ...
            print(
                f"Training data saved to training_data/training_data_{timestamp}.parquet"
            )
        return training_df
...
test_messages = [
    ...
    HumanMessage(content="What is a superconducting qubit?"),
]
...
def test_chat_structured_output(chat_model: BaseChatModel) -> None:
    ...
    try:
        ...
        assert isinstance(resp, TestStructuredOutput)
        print(
            "Congratulations! The model successfully produced the test structured output."
        )
        print(f"Output: {resp}")
    except Exception as e:
        raise AssertionError(f"Structured output failed: {e}")
...
def test_chat_tool_calls(
    ...
    try:
        ...
        tool_call_data = read_tool_calls(resp)
        try:
            output = TestStructuredOutput.model_validate(tool_call_data)
            print(f"Tool call successful: {output}")
        except Exception as e:
            raise AssertionError(
                f"The model made a tool call but the output was invalid: {e}"
            )
        print(
            "Congratulations! The model successfully ran the tool call and produced the test structured output."
        )
    except Exception as e:
        raise AssertionError(f"Tool call failed: {e}")
